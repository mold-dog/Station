...
Public Class Paint_Production_Quality
    ...
    Private Sub Update_Screen()
        ...
        query = "exec Paint_Conveyor_Paint_Runs_Quality '" & DTP_Start_Date.Text & " " & DTP_Start_Time.Text & "', '" & DTP_End_Date.Text & " " & DTP_End_Time.Text & "'"
    ...
    Private Sub DGV_Paint_Data_RowHeaderMouseDoubleClick(sender As Object, e As DataGridViewCellMouseEventArgs) Handles DGV_Paint_Data.RowHeaderMouseDoubleClick
        ...
        If IsDate(DGV_Paint_Data.Rows(e.RowIndex).Cells(3).Value & "") Then
            end_date = DateAdd(DateInterval.Second, 1, DGV_Paint_Data.Rows(e.RowIndex).Cells(3).Value)
        Else
            end_date = Format(Now(), "yyyy/MM/dd hh:mm:ss")
        End If
        Query = "exec Paint_Conveyor_Paint_Runs_Quality_Parts '" & DGV_Paint_Data.Rows(e.RowIndex).Cells(2).Value & "', '" & end_date & "'"
